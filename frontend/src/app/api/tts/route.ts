...
export async function POST(request: NextRequest) {
  try {
    ...
    const voiceName: string | undefined = body.voiceName; // e.g., en-US-Standard-C or en-US-Neural2-A
  const speakingRate: number | undefined = (typeof body.speakingRate === 'number') ? body.speakingRate : undefined; // 0.25 to 4.0
  const pitch: number | undefined = (typeof body.pitch === 'number') ? body.pitch : undefined; // -20.0 to 20.0
    ...
    const payload = {
      ...
      voice: {
        languageCode,
        name: voiceName, // optional; Google will choose a default if not specified
      },
      audioConfig: {
